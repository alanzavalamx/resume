...
// Elegante portafolio/CV en una sola página para Alan Zavala.
// ▸ Usa Tailwind (clases) y React puro (sin dependencias externas).
// ▸ Para publicar: puedes copiar este archivo a un proyecto Vite/Next o usarlo directo aquí.
// ▸ TODO: Actualiza los enlaces de LinkedIn/GitHub y el href del CV si cambias la ruta del PDF.

export default function Portfolio() {
  // ====== Datos (edita libremente) ======
  const CONTACT = {
    ...
    // Reemplaza con tus enlaces reales cuando gustes
    linkedin: "https://www.linkedin.com/in/tu-usuario", // TODO
    github: "https://github.com/tu-usuario", // TODO
    // Si vas a hospedar el PDF, actualiza este enlace. Durante la edición aquí puedes mantener el nombre del archivo.
    cvHref: "Data Engineer Alan Zavala.pdf", // o por ejemplo "/cv/alan-zavala.pdf"
  };
  ...
  const EDUCATION = [
    {
      degree: "Maestría en Inteligencia Artificial Aplicada",
      school: "Tecnológico de Monterrey",
    },
    {
      degree: "Ingeniería Electrónica y Automatización",
  ...
  const PROJECTS = [
    {
      ...
      description:
        "Pipeline OT→Cloud para capturar telemetría, detectar anomalías y reducir scrap.",
      tags: ["IoT", "AWS", "PySpark", "MLOps"],
      link: "#", // TODO: agrega URL del proyecto/demo
    },
    {
      title: "Data Lakehouse para analítica avanzada",
      description:
        "Arquitectura Lakehouse con ingestión batch/stream, catálogo y gobernanza.",
    ...
    },
    {
      ...
      description:
        "Modelos de pronóstico para optimizar inventarios y planeación de producción.",
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100 antialiased">
      {/* Meta simple para SEO cuando se incruste en un HTML */}
        ...
        <nav className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-2xl border border-neutral-300 dark:border-neutral-700 px-3 py-1.5 text-sm hover:bg-neutral-100 dark:hover:bg-neutral-900"
            >
            ...
            <button
              ...
              onClick={toggleTheme}
              className="inline-flex items-center rounded-2xl border border-neutral-300 dark:border-neutral-700 p-1.5 hover:bg-neutral-100 dark:hover:bg-neutral-900"
            >
              <ThemeIcon dark={dark} />
            </button>
          </div>
      ...
      <style>{`
        ...
        .card { background: white; border: 1px solid rgba(0,0,0,0.08); border-radius: 1rem; padding: 1.25rem; box-shadow: 0 1px 0 0 rgba(0,0,0,0.03); }
        .dark .card { background: rgb(15 15 15); border-color: rgb(38 38 38); }
        .chip { display: inline-flex; align-items: center; gap: .5rem; border: 1px solid rgba(0,0,0,0.12); padding: .4rem .7rem; border-radius: 999px; }
        .dark .chip { border-color: rgb(64 64 64); }
        .tag { font-size: .75rem; border: 1px solid rgba(0,0,0,0.12); padding: .2rem .55rem; border-radius: 999px; }
        .dark .tag { border-color: rgb(64 64 64); }
      `}</style>
    </div>
  );
}
...
function Meta({ contact }) {
  // Inyecta etiquetas cuando se inserte dentro de un <head> real
  return (
    <>
...
function formatPhone(phone) {
  // Simple pretty print for MX numbers: +52 81 2354 0768 → +52 81 2354 0768
